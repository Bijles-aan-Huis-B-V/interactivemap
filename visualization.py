...
def index():
    ...
    map_html = df_html = selected_country = None
    selected_school_levels = selected_school_years = selected_school_types = selected_course_names = []
    selected_availabilities = selected_tutor_types = selected_excluded = []
    lessons_per_relation = no_lesson_tutor = start_date = end_date = None
    use_location_filter = latitude_input = longitude_input = distance_km = None
            ...
            if not Tutor_filtered.empty:
                ...
                for _, row in Tutor_filtered.iterrows():
                    ...
                    circle = folium.Circle(
                        ...
                        popup=f"""<div class='circle-popup'>Tutor: {row['tutor']}<br>Availability: {row['availability']}<br>Tutor type: {row['tutor_category']}<br>Excluded: {row['excluded_from_search']}<br>Total lessons: {row['total_lessons']}<br>Per relation: {row['lessons_per_relation']}<br>Link: <a href=\"{'https://bijlesaanhuis.nl/profiel/' if row['country'] == 'nl' else 'https://lernigo.de/profil/'}{row['tutor']}\">Profile</a></div>""",
                        fill=True,
                        fill_opacity=0,
                    ).add_to(tutors_map)
